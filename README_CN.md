...
在页面导入 `auth.json`，点击「刷新用量与重置机会」。也可以把文件放进项目目录，服务会在空闲时读取文件变化。网页当前使用中文界面。
...
服务监听 `127.0.0.1`，校验请求来源，并要求修改操作携带本地会话令牌。凭证令牌不会显示在网页或进程参数中。请求发往固定的 ChatGPT 额度接口和 OpenAI OAuth 令牌接口。上述本地数据文件已列入 `.gitignore`。
